...
from robomimic.utils.demo_utils import DemoRecorder
...
def playback_trajectory_with_env(
    ...
    env.env.set_perturbation_values_from_array(init_perturb)
    env.env.reset()
    ...
    # TEMP: Keeping track of min and max force
    max_force = 0.0
    min_force = 100.0
    ...
    for i in range(traj_len):
        if action_playback:
            ob_dict, r, done, _ = env.step(actions[i])
            left_ft_recent = ob_dict['robot0_robot1_forcetorque-state'][...,-1,:6]
            right_ft_recent = ob_dict['robot0_robot1_forcetorque-state'][...,-1,6:]

            # max_force_recent = np.max([np.max(left_ft_recent[:3]), np.max(right_ft_recent[:3])])
            # min_force_recent = np.min([np.min(left_ft_recent[:3]), np.min(right_ft_recent[:3])])
            # max_torque_recent = np.max([np.max(left_ft_recent[3:]), np.max(right_ft_recent[3:])])
            # min_torque_recent = np.min([np.min(left_ft_recent[3:]), np.min(right_ft_recent[3:])])

            # if max_force < max_force_recent:
            #     max_force = max_force_recent
            # if min_force > min_force_recent:
            #     min_force = min_force_recent
            # if max_torque < max_torque_recent:
            #     max_torque = max_torque_recent
            # if min_torque > min_torque_recent:
            #     min_torque = min_torque_recent
            ...
            ob_dict = env.reset_to({"states" : states[i]})
            left_ft_recent = ob_dict['robot0_robot1_forcetorque-state'][...,-1,:6]
            right_ft_recent = ob_dict['robot0_robot1_forcetorque-state'][...,-1,6:]
        ...
        if write_video:
            if video_count % video_skip == 0:
                ...
                video_writer.append_data(recorder.step(video_img, left_ft_recent, right_ft_recent))
            video_count += 1
...
def playback_dataset(args):
    ...
    for ind in range(len(demos)):
        ...
        if is_robosuite_env:
            ...
            initial_state["init_perturb"] = f["data/{}".format(ep)].attrs["init_perturb"]
