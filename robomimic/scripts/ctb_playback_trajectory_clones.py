...
def playback_trajectory_with_env(
    ...
    for env_count in range(5):
        eps_actions = actions.copy()
        print("PLAYING VARIATION", env_count)
        recorder = DemoRecorder()
        env.env.reset()
        for i in range(traj_len):
            print(eps_actions[i])
            ob_dict, r, done, _ = env.step(eps_actions[i])
            ...
            if video_count % video_skip == 0:
                ...
                video_imgs[str(env_count)].append(recorder.step(video_img, left_ft_recent, right_ft_recent))
            video_count += 1

    for a, b, c, d in zip(video_imgs['0'], video_imgs['1'], video_imgs['2'], video_imgs['3']):
        full_img = np.vstack((a,b,c,d))
        video_writer.append_data(full_img)
...
def playback_dataset(args):
    ...
    # Allow environment to incorporate object and shape variations into playback
    env_meta['env_kwargs']['peg_perturbation'] = None
    env_meta['env_kwargs']['hole_perturbation'] = None
    # env_meta['env_kwargs']['obj_shape'] = None
    env_meta['env_kwargs']['obj_variations'] = ['xt', 'zt', 'yr', 'zr']
    # env_meta['env_kwargs']['obj_shape_variations'] = ['arrow', 'circle', 'cross', 'diamond', 'hexagon', 'key', 'line', 'pentagon', 'u']
    env_meta['env_kwargs']['peg_body_shape'] = None
    env_meta['env_kwargs']['hole_body_shape'] = None
    env_meta['env_kwargs']['obj_body_shape_variations'] = ["cube", "cylinder"]
    env_meta['env_kwargs']['peg_hole_swap'] = None
    env_meta['env_kwargs']['visual_variations'] = ['texture', 'arena-train']
