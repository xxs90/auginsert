...
class RolloutPolicy(object):
    ...
    def _prepare_observation(self, ob):
        ...
        """
        # TEMP: Normalize forcetorque by processing data
        for k in self.policy.global_config.all_obs_keys:
            if 'forcetorque' in k:
...
class VTTRolloutPolicy(object):
    ...
    def __call__(self, ob, goal=None):
        ...
        if goal is not None:
            goal = self._prepare_observation(goal)
        ac, img_attn, tactile_attn, proportions = self.policy.get_attention(obs_dict=ob, goal_dict=goal)
        return TensorUtils.to_numpy(ac[0]), img_attn, tactile_attn, proportions
