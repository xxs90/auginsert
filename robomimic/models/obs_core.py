...
import robomimic.models.encoders.ft_encoders as ForceTorqueEncoders
...
class VisualCore(EncoderCore, BaseNets.ConvBase):
    ...
    def forward(self, inputs):
        ...
        x = super(VisualCore, self).forward(inputs)
        # print("RGB |", "max", torch.max(x).item(), "min", torch.min(x).item(), "mean", torch.mean(x).item())
        return x
...
class LowDimEncoderCore(EncoderCore):
    ...
    def __init__(
        ...
        assert encoder_type in ["mlp", "rnn", "causalconv", "selfattn", "transformer"]

        if 'output_dim' in encoder_kwargs.keys():
            assert encoder_kwargs['output_dim'] == output_dim, "These output shapes should match!"

        # Always need to run super init first and pass in input_shape
        super().__init__(input_shape=input_shape)

        self.output_shape_raw = output_dim
        self.encoder_type = encoder_type

        if encoder_type == "mlp":
            self.encoder = BaseNets.MLP(
                input_dim=input_shape[-1], 
                **encoder_kwargs
            )
        elif encoder_type == "rnn":
            self.encoder = BaseNets.RNN_Base(
                input_dim=input_shape[-1],
                **encoder_kwargs
            )
        elif encoder_type == "causalconv":
            self.encoder = BaseNets.CausalConv1dBase(
                input_channel=input_shape[-1],
                **encoder_kwargs
            )
        elif encoder_type == "selfattn":
            self.encoder = ForceTorqueEncoders.CausalForceTorqueEncoder(
                input_dim = input_shape[-1],
                **encoder_kwargs
            )
        elif encoder_type == "transformer":
            self.encoder = ForceTorqueEncoders.TransformerForceTorqueEncoder(
                input_dim = input_shape[-1],
                **encoder_kwargs
            )
    ...
    def output_shape(self, input_shape=None):
        # shape = self.encoder.output_shape(input_shape=input_shape)
        shape = [self.output_shape_raw]
        return shape

    # we also need to specify the forward pass for this network
    def forward(self, inputs):
        # Take only most current observation for MLP
        # NOTE: Only do this for input force-torque histories
        # if self.encoder_type == "mlp" and len(inputs.shape) > 2:
        #     inputs = inputs[...,-1,:]
        
        if self.encoder_type == "causalconv":
            # Reshape inputs so that history length = channel size
            inputs = inputs.permute((0,2,1))

        outputs = self.encoder.forward(inputs)

        # Take only last hidden state output for RNN
        if self.encoder_type == "rnn":
            outputs = outputs[...,-1,:]

        # Remove trailing dimension off of causalconv
        if self.encoder_type == "causalconv":
            outputs = outputs.squeeze(-1)

        # # remove trailing dimensions off of selfattn
        # if self.encoder_type == "selfattn":
        #     outputs = outputs.squeeze(dim=(-2,-1))
        return outputs
