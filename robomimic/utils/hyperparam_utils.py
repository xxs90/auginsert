...
class ConfigGenerator(object):
    ...
    """
    def __init__(self, base_config_file, wandb_proj_name="debug", script_file=None, generated_config_dir=None, num_splits=4):
        """
        Args:
    ...
    def _script_from_jsons(self, json_paths):
        ...
        for json_path in json_paths:
            splits[spl].append(json_path)
            spl = (spl + 1) % num_split
        
        for n, split in enumerate(splits):
            if len(split) > 0:
                with open(self.script_file.split('.')[0] + str(n) + '.sh', 'w') as f:
                    f.write("#!/bin/bash\n\n")
                    for path in split:
                        # write python command to file
                        cmd = "python robomimic/scripts/train.py --config {}\n".format(path)
                        
                        print()
                        print(cmd)
                        f.write(cmd)

        # json_per_split = len(json_paths) // num_split
        # if json_per_split > 0:
        #     for split in range(num_split):
        #         json_split = json_paths[json_per_split*split:json_per_split*(split+1)]
        #         with open(self.script_file.split('.')[0] + str(split) + '.sh', 'w') as f:
        #             f.write("#!/bin/bash\n\n")
        #             for path in json_split:
        #                 # write python command to file
        #                 cmd = "python robomimic/scripts/train.py --config {}\n".format(path)
