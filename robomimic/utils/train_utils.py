...
def load_data_for_training(config, obs_keys):
    ...
    Args:
        ...
        obs_keys (list): list of observation lities that are required for
...
def run_rollout(
    ...
    try:
        for step_i in range(horizon):
            ...
            if vtt_policy:
                ac, img_attn, tactile_attn, proportions = policy(ob=ob_dict)
            ...
            ft = ob_dict['robot0_robot1_forcetorque-state']
            if len(ft.shape) == 2:
                left_ft = ft[:,:6]
                right_ft = ft[:,6:]
            else:
            ...
            ft = ob_dict['robot0_robot1_ft_denoised-state']

            # If force/torque readings are above maximum thresholds, fail the demo early
            ...
            if video_writer is not None:
                if video_count % video_skip == 0:
                    if vtt_policy:
                        ...
                        # top_video_img = env.render(mode="rgb_array", camera_name="overhead", height=480, width=640)
                        front_video_img = env.render(mode="rgb_array", camera_name="frontview", height=480, width=640)

                        # Add heatmap on top of image observations
                        overhead = (ob_dict['overhead_image'].transpose((1,2,0)) * 255.0).astype(np.uint8)
                        l_wristview = (ob_dict['left_wristview_image'].transpose((1,2,0)) * 255.0).astype(np.uint8)
                        r_wristview = (ob_dict['right_wristview_image'].transpose((1,2,0)) * 255.0).astype(np.uint8)
                        super_imposed_imgs = [
                            cv2.addWeighted(heatmap, 0.5, a, 0.5, 0) for heatmap, a in zip(heatmaps, [l_wristview, overhead, r_wristview])
                        ]
                        ...
                        top_video_img = env.render(mode="rgb_array", camera_name="overhead", height=480, width=640)
                        front_video_img = env.render(mode="rgb_array", camera_name="frontview", height=480, width=640)
                        gripper_view = env.render(mode="rgb_array", camera_name="left_wristview", height=480, width=640)
                        all_frames = recorder.step([top_video_img, front_video_img], left_ft, right_ft, ac)
