...
class DepthModality(Modality):
    ...
    def _default_obs_processor(cls, obs):
        ...
        """
        # NOTE: Scale calibrated for CapTheBottle environment
...
class ForceTorqueModality(Modality):
    ...
    @classmethod
    def _default_obs_processor(cls, obs):
        assert obs.shape[-1] == 6 or obs.shape[-1] == 12
        if obs.shape[-1] == 6:
            obs[...,:3] /= 100
            obs[...,3:] /= 2
        elif obs.shape[-1] == 12:
        ...
        if isinstance(obs, np.ndarray):
            ...
            raise Exception(f"No way to process force-torque of type {type(obs)}!")
        return obs
