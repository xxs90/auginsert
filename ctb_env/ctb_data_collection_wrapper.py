...
class CapTheBottleDataCollectionWrapper(DataCollectionWrapper):
    def __init__(self, env, directory, collect_freq=1, flush_freq=100):
        if isinstance(env, Wrapper):
            assert isinstance(env.env, CapTheBottleInitializer), "This wrapper only works with a wrapped CapTheBottleInitializer environment!"
        else:
            assert isinstance(env, CapTheBottleInitializer), "This wrapper only works with the CapTheBottleInitializer environment!"
...
class DualArmCapTheBottleDataCollectionWrapper(DataCollectionWrapper):
    def __init__(self, env, directory, collect_freq=1, flush_freq=100):
        if isinstance(env, Wrapper):
            assert isinstance(env.env, CapTheBottleInitializer), "This wrapper only works with a wrapped CapTheBottleInitializer environment!"
        else:
