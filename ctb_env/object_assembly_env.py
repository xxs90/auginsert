...
class ObjectAssembly(TwoArmEnv):
    def __init__(
            self,
            robots=["UR5e", "UR5e"],
        ...
        # Sete new base position for left and right arms
        self.base_left_pose = np.array([0, -0.0465, 0.625, -1.57, 0, 0, 1],
                                       dtype=np.float64)  # 0, -0.0465, 0.625, -1.57, 0, 0, 1
        self.base_right_pose = np.array([0, 0.0465, 0.625, 1.57, 0, 0, 1],
                                        dtype=np.float64)  # 0, 0.0465, 0.625, 1.57, 0, 0, 1
        ...
        print("[ObjectAssembly] Environment initialized")
    ...
    def _load_model(self):
        print("[ObjectAssembly]: Loading model...")
        super()._load_model()

        # Setup robot arms
        # for robot, offset in zip(self.robots, (-0.5, 0.5)):
        # the offset is the distance for base, the abs(value), value larger, arm further
        for robot, offset in zip(self.robots, (-0.8, 0.8)):
            xpos = robot.robot_model.base_xpos_offset["empty"]
            xpos = np.array(xpos) + np.array((0, offset, 0.5))
            robot.robot_model.set_base_xpos(xpos)
            print(f"[ObjectAssembly]: xpos: {xpos}")

        self.robots[0].init_qpos = self.init_robot0_qpos
        self.robots[1].init_qpos = self.init_robot1_qpos

        # this is the orientation for the base, (0, 0, 0) is placing on the table (top-down view)
        ...
        camera_attribs["quat"] = array_to_string(
            T.quat_multiply(np.array([0, 0.707108, 0.707108, 0]), T.mat2quat(T.euler2mat(np.array([0, -0.26, 0])))))
    ...
    def _setup_observables(self):
        ...
        def ft_all_denoised(obs_cache):
            ft_curr = np.hstack(
                [self.robots[0].ee_force, self.robots[0].ee_torque, self.robots[1].ee_force, self.robots[1].ee_torque])
        ...
        def prop_all(obs_cache):
            ...
            l_eef_quat = T.convert_quat(self.robots[0].sim.data.get_body_xquat(self.robots[0].robot_model.eef_name),
                                        to="xyzw")
            r_eef_pos = np.array(self.robots[1].sim.data.site_xpos[self.robots[1].eef_site_id])
            r_eef_quat = T.convert_quat(self.robots[1].sim.data.get_body_xquat(self.robots[1].robot_model.eef_name),
                                        to="xyzw")
...
class ObjectAssemblyInitializer(ObjectAssembly):
    ...
    TRAIN_ARENA_TEXTURES = ['blue-wood', 'brass-ambra', 'ceramic', 'clay', 'dark-wood',
                            'light-wood']  # light-wood is default
    EVAL_ARENA_TEXTURES = ['cream-plaster', 'gray-felt', 'gray-plaster', 'gray-woodgrain', 'light-gray-floor-tile',
                           'light-gray-plaster', 'metal', 'pink-plaster', 'red-wood', 'steel-brushed',
                           'steel-scratched',
    ...
    def __init__(
            ...
            obj_body_shape_variations=None,
            # (list) List of possible shapes if generating randomized object body shapes
            pose_variations=None,  # (list) Can be either "trans", "rot", or both if generating randomized poses
            obj_variations=None,
            # (list) Can be a subset of ["xt", "zt", "yr", "zr"] if generating randomized obj poses
            visual_variations=None,
            # (list) Can be a subset of ["camera", "lighting", "texture", "arena-train", "arena-eval"] for generating visual variations
            perturbation_seed=None,  # (int) Deterministic seed for randomized variations
            **kwargs  # Arguments for ObjectAssembly environment
    ):
        super().__init__(**kwargs)
        # Set initial perturbation variables (base left/right arm and ft noise stds are set in ObjectAssembly)
        self.obj_shape = None
        ...
        self.obj_shape_variations = obj_shape_variations if obj_shape_variations is not None else ObjectAssemblyInitializer.ENV_PEG_HOLE_SHAPES
        self.obj_body_shape_variations = obj_body_shape_variations if obj_body_shape_variations is not None else ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES
        ...
        if perturbation_seed is None:
            ...
            print(f"[ObjectAssemblyInitializer]: Using seed {perturbation_seed} for initializations")
            self.obj_perturbation_seed = perturbation_seed + 99

        # Initialize random generators
        self.rngs = {
            'base_pose_rng': np.random.default_rng(seed=self.perturbation_seed),
            # Used for generating base insertion poses
            ...
            'noise_rng': np.random.default_rng(seed=self.perturbation_seed),
            # Used for generating randomized ft noise stds
            ...
            'obj_shape_rng': np.random.default_rng(seed=self.obj_perturbation_seed),
            # Used for generating peg/hole shapes
            'obj_body_shape_rng': np.random.default_rng(seed=self.obj_perturbation_seed)
            # Used for generating object body shapes
        }

        if obj_shape is not None:
            assert obj_shape in ObjectAssemblyInitializer.ENV_PEG_HOLE_SHAPES, f'Given peg/hole shape must be one of {ObjectAssemblyInitializer.ENV_PEG_HOLE_SHAPES}'
        ...
        if peg_body_shape is not None:
            assert peg_body_shape in ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES, f'Given peg shape must be one of {ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES}'
            self.peg_body_shape = peg_body_shape
        else:
            self.randomized_values.append("peg_body_shape")

        if hole_body_shape is not None:
            assert hole_body_shape in ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES, f'Given hole shape must be one of {ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES}'
    ...
    def _set_init_state(self):
        ...
        if "obj_shape" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Object shape not given, generating random...")
            self.obj_shape = self._generate_randomized_obj_shape()
        else:
            print("ObjectAssemblyInitializer]: Initializing with given object shape")

        if "peg_body_shape" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Peg body shape not given, generating random...")
            self.peg_body_shape = self._generate_randomized_obj_body_shape(peg=True)
        else:
            print("[ObjectAssemblyInitializer]: Initializing with given peg body shape")

        if "hole_body_shape" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Hole body shape not given, generating random...")
            self.hole_body_shape = self._generate_randomized_obj_body_shape(peg=False)
        else:
            print("[ObjectAssemblyInitializer]: Initializing with given peg body shape")

        if "peg_hole_swap" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Potentially swapping peg and hole...")
            self.peg_hole_swap = self._generate_randomized_peg_hole_swap()

        if "base_pose" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Base insertion pose not given, generating random...")
            self.base_left_pose, self.base_right_pose = self._generate_randomized_base_poses()
        else:
            print("[ObjectAssemblyInitializer]: Initializing with given base insertion pose")

        if "base_pose_perturb" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Left and right arm pose not given, generating random...")
            self.left_perturb, self.right_perturb = self._generate_randomized_poses()
        else:
            print("[ObjectAssemblyInitializer]: Initializing with given left and right arm pose")

        if "peg_perturb" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Peg pose not given, generating random...")
            ...
            print("[ObjectAssemblyInitializer]: Initializing with given peg pose")

        if "hole_perturb" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Hole pose not given, generating random...")
            self.hole_perturb = self._generate_randomized_obj_pose()
        else:
            print("[ObjectAssemblyInitializer]: Initializing with given hole pose")

        if "arena_texture" in self.randomized_values:
            print("[ObjectAssemblyInitializer]: Randomizing arena texture...")
        ...
        print(
            f'''
            ====== [ObjectAssemblyInitializer]: Episode initialization ======
    ...
    def _step_to_init_pose(self):
        print("[ObjectAssemblyInitializer] Setting initial robot pose...")
        ...
        action = np.hstack((left_action, right_action))
        for _ in range(40):
            super(ObjectAssembly, self).step(action)
    ...
    def _generate_randomized_obj_pose(self):
        ...
        random_obj_pose = np.array(
            [0, random_obj_pose[0], random_obj_pose[1], random_obj_pose[3], 0, random_obj_pose[2]], dtype=np.float64)
    ...
    def _arm_perturb_from_obj(self, peg_variations, hole_variations):
        ...
        peg_correction = ObjectAssemblyInitializer._convert_perturb_zr(peg_correction, peg=True)

        xt_hole, zt_hole, yr_hole, zr_hole = hole_variations[1], hole_variations[2], hole_variations[5], \
        hole_variations[3]
        ...
        hole_correction = ObjectAssemblyInitializer._convert_perturb_zr(hole_correction, peg=False)
    ...
    def set_perturbation_values_from_array(self, init_perturb):
        ...
        obj_shape, peg_body_shape, hole_body_shape, swap = init_perturb[27], init_perturb[28], init_perturb[29], \
        init_perturb[30]
    ...
    def set_perturbation_values(
        ...
        if obj_shape is not None:
            if isinstance(obj_shape, str):
                assert obj_shape in ObjectAssemblyInitializer.ENV_PEG_HOLE_SHAPES
                self.obj_shape = obj_shape
            else:
                self.obj_shape = ObjectAssemblyInitializer.ENV_PEG_HOLE_SHAPES[int(obj_shape)]
            if "obj_shape" in self.randomized_values:
                self.randomized_values.remove("obj_shape")

        if peg_body_shape is not None:
            if isinstance(peg_body_shape, str):
                assert peg_body_shape in ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES
                self.peg_body_shape = peg_body_shape
            else:
                self.peg_body_shape = ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES[int(peg_body_shape)]
            if "peg_body_shape" in self.randomized_values:
                self.randomized_values.remove("peg_body_shape")

        if hole_body_shape is not None:
            if isinstance(hole_body_shape, str):
                assert hole_body_shape in ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES
                self.hole_body_shape = hole_body_shape
            else:
                self.hole_body_shape = ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES[int(hole_body_shape)]
            if "hole_body_shape" in self.randomized_values:
    ...
    def get_perturbation_values(self):
        ...
        return {
            ...
            'obj_shape': np.array([ObjectAssemblyInitializer.ENV_PEG_HOLE_SHAPES.index(self.obj_shape)],
                                  dtype=np.float64),
            'peg_body_shape': np.array([ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES.index(self.peg_body_shape)],
                                       dtype=np.float64),
            'hole_body_shape': np.array([ObjectAssemblyInitializer.ENV_OBJ_BODY_SHAPES.index(self.hole_body_shape)],
                                        dtype=np.float64),
    ...
    def randomize_visuals(self):
        if len(self.domain_randomizers) <= 0:
            return
        print("[ObjectAssemblyInitializer] Generating visual variations")
    ...
    def reset(self, seed=None):
        ...
        if self.reset_counter > 0:
            ...
            print("[ObjectAssemblyInitializer]: No steps taken, avoiding reset...")
    ...
    def reset_perturbation_generator(self, seed=None):
        ...
        print("[ObjectAssemblyInitializer]: Resetting initial perturbation generator")
