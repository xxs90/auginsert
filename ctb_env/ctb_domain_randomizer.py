...
class LightingModder(BaseModder):
    ...
    def update_sim(self, sim):
        super().update_sim(sim)
        self.light_names = self.sim.model.light_names
...
class CameraModder(BaseModder):
    ...
    def __init__(
        ...
        position_perturbation_size=0.04,
    ...
    def update_sim(self, sim):
        super().update_sim(sim)
        self.camera_names = self.sim.model.camera_names
...
class TextureModder(BaseModder):
    ...
    def __init__(
        ...
        if geom_names is None:
            geom_names = [n for n in self.sim.model.geom_names if "peg" in n or "hole" in n] # self.sim.model.geom_names
    ...
    def update_sim(self, sim):
        super().update_sim(sim)
        self.geom_names = [n for n in self.sim.model.geom_names if "peg" in n or "hole" in n] # self.sim.model.geom_names
